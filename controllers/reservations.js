...
exports.addReservation = async (req, res, next) => {
    try {
        ...
        if(existedReservations.length >= 3 && req.user.role != 'admin'){
            return res.status(400).json({success: true, message: `The user with ID ${req.user.id} has already made 3 reservations.`})
        }
...
exports.deleteReservation = async (req, res, next) => {
    try {
        ...
        if(!reservation){
            return res.status(404).json({success: true, message: `No reservation with id ${req.params.id}`});
        }
